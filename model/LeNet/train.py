...
class Trainer:
    def __init__(self, batch_size=100, epochs=200, lr=0.001,save_path="weights".format(datetime.datetime.now())):
        ...
        train_set = torchvision.datasets.MNIST(root="./data", train=True, transform=self.transform, download=True)
        val_set = torchvision.datasets.MNIST(root="./data", train=False, transform=self.transform, download=True)
    ...
    def run(self):
        ...
        for epoch in range(self.epochs):
            ...
            for step, data in enumerate(train_bar):
                ...
                loss.backward()
                self.optimizer.step()
                train_bar.desc = "train epoch[{}/{}] loss:{:.3f}".format(epoch + 1, self.epochs, running_loss/(step+1))
